...
const MenuLink = ({ to, label, active = false }: { to: string, label: string, active?: boolean }) => (
...
export const Sidebar: React.FC = () => {
  return (
    ...
    >
      {/* Logo Area */}
      <Flex h="80px" align="center" justify="center" borderBottom="1px solid" borderColor="whiteAlpha.50" mb={2}>
        <Text fontSize="xl" fontWeight="black" color="white" letterSpacing="wide" fontStyle="italic">
          PODIUM <Text as="span" color="brand.100">GO</Text>
        </Text>
      </Flex>

      {/* Menu Scrollável */}
      <Box overflowY="auto" h="calc(100vh - 80px)" css={{
        '&::-webkit-scrollbar': { width: '4px' },
        '&::-webkit-scrollbar-track': { background: 'transparent' },
        '&::-webkit-scrollbar-thumb': { background: '#2c3e50', borderRadius: '4px' },
      }}>
        <VStack spacing={0} align="stretch">
          ...
          <MenuSection title="Operação (Frota)" />
          <MenuLink to="/drivers" label="Motoristas" />
          <MenuLink to="/map" label="Mapa em Tempo Real" />
          <MenuLink to="/rides" label="Corridas" />

          <MenuSection title="Gestão B2B" />
          <MenuLink to="/companies" label="Empresas & CCs" />
          <MenuLink to="/employees" label="Funcionários" />

          <MenuSection title="Financeiro" />
          <MenuLink to="/pricing" label="Tabelas de Preço" />
          <MenuLink to="/invoices" label="Faturamento" />

          <MenuSection title="Sistema" />
          <MenuLink to="/settings" label="Configurações" />
          <MenuLink to="/audit" label="Auditoria" />
